...
function buildTranscript(chat, botName, sessionId) {
  ...
  (chat.messages || []).forEach(m => {
    lines.push(`[${m.role === 'user' ? 'Visitor' : botName}]: ${m.content}\n`);
  });
...
const server = http.createServer(async (req, res) => {
  ...
  if (p === '/api/bots' && m === 'POST') {
    ...
    const bot = {
      ...
      allowed_origins: b.allowed_origins || '', calendar_link: b.calendar_link || '',
  ...
  if (botM && m === 'PUT') {
    ...
    const u = {
      ...
      allowed_origins: b.allowed_origins ?? bot.allowed_origins, calendar_link: b.calendar_link ?? bot.calendar_link,
  ...
  if (avM && m === 'POST') {
    try {
      ...
      if (file.data.length > 5 * 1024 * 1024) return json(res, { error: 'Max 5MB' }, 400);
      const fname = 'avatar_' + avM[1] + ext;
      fs.writeFileSync(path.join(UPLOADS_DIR, fname), file.data);
      const avatarUrl = '/uploads/' + fname;
      const bots = getBots();
      if (bots[avM[1]]) { bots[avM[1]].avatar_url = avatarUrl; saveBots(bots); }
      return json(res, { url: avatarUrl });
  ...
  if (infoM && m === 'GET') {
    ...
    return json(res, { name: bot.name, welcome_msg: bot.welcome_msg, avatar_url: bot.avatar_url || '', widget_color: bot.widget_color, widget_pos: bot.widget_pos, calendar_link: bot.calendar_link || '' });
  }
