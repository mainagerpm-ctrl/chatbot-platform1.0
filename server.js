...
// Use /tmp on Railway (always writable), fall back to local data dir
const DATA_DIR    = process.env.RAILWAY_ENVIRONMENT ? '/tmp/chatbot-data' : path.join(__dirname, 'data');
const UPLOADS_DIR = process.env.RAILWAY_ENVIRONMENT ? '/tmp/chatbot-uploads' : path.join(__dirname, 'uploads');
...
const server = http.createServer(async (req, res) => {
  ...
  if (p === '/api/bots' && m === 'POST') {
    ...
    } catch(e) { console.error('POST /api/bots error:', e); return json(res, { error: e.message }, 500); }
  }
